...
colnames2use = c("Sex_Fstat", "Sex_pval","Sex_fdrq","Sex_EffectSize_MvsF",
                 ...
                 "ASDm_sd","ASDf_sd","TDm_sd","TDf_sd",
                 "ASDm_m","ASDf_m","TDm_m","TDf_m",
                 "ASDm_cov","ASDf_cov","TDm_cov","TDf_cov",
                 "LeveneF_TDsex","LevenePval_TDsex",
                 "LeveneF_ASDsex","LevenePval_ASDsex")
...
for (icol in 1:ncols2use) {
  ...
  output_res[icol,"ASDm_sd"] = sd(asdm_data[,region_labels[icol]], na.rm=FALSE)
  output_res[icol,"ASDf_sd"] = sd(asdf_data[,region_labels[icol]], na.rm=FALSE)
  output_res[icol,"TDm_sd"] = sd(tdm_data[,region_labels[icol]], na.rm=FALSE)
  output_res[icol,"TDf_sd"] = sd(tdf_data[,region_labels[icol]], na.rm=FALSE)
  
  output_res[icol,"ASDm_m"] = mean(asdm_data[,region_labels[icol]], na.rm=FALSE)
  output_res[icol,"ASDf_m"] = mean(asdf_data[,region_labels[icol]], na.rm=FALSE)
  output_res[icol,"TDm_m"] = mean(tdm_data[,region_labels[icol]], na.rm=FALSE)
  output_res[icol,"TDf_m"] = mean(tdf_data[,region_labels[icol]], na.rm=FALSE)

  output_res[icol,"ASDm_cov"] = output_res[icol,"ASDm_sd"]/output_res[icol,"ASDm_m"]
  output_res[icol,"ASDf_cov"] = output_res[icol,"ASDf_sd"]/output_res[icol,"ASDf_m"]
  output_res[icol,"TDm_cov"] = output_res[icol,"TDm_sd"]/output_res[icol,"TDm_m"]
  output_res[icol,"TDf_cov"] = output_res[icol,"TDf_sd"]/output_res[icol,"TDf_m"]
    
  res = leveneTest(td_data[,region_labels[icol]], td_data$Sex)
  output_res[icol,"LeveneF_TDsex"] = res[1,"F value"]
  output_res[icol,"LevenePval_TDsex"] = res[1,"Pr(>F)"]
  res = leveneTest(asd_data[,region_labels[icol]], asd_data$Sex)
  output_res[icol,"LeveneF_ASDsex"] = res[1,"F value"]
  output_res[icol,"LevenePval_ASDsex"] = res[1,"Pr(>F)"]
...
output_res$fdrq_Levene_TDsex = p.adjust(output_res$LevenePval_TDsex, method = "fdr")
output_res$fdrq_Levene_ASDsex = p.adjust(output_res$LevenePval_TDsex, method = "fdr")
...
```{r, warning=FALSE, message=FALSE}
tmp_res = output_res[order(output_res$LevenePval_TDsex),]
vars2use = c("area_descrip",
             "TDf_cov","TDm_cov","ASDf_cov","ASDm_cov",
             "LeveneF_TDsex","LevenePval_TDsex","fdrq_Levene_TDsex",
             "LeveneF_ASDsex","LevenePval_ASDsex","fdrq_Levene_ASDsex")
kable(tmp_res[,vars2use])

lims2use = c(0.04,0.28)

p1 = ggseg(.data = output_res, 
          mapping=aes(fill=TDf_cov), 
          position="stacked", 
          atlas=glasser) + 
  ggtitle("TD Female CoV") + 
  theme(plot.title = element_text(hjust = 0.5)) + 
  guides(fill=guide_legend(title="SD")) + 
  scale_fill_gradient2(midpoint=0, low="blue", mid="white", high="red", limits=lims2use, breaks = round(seq(from=lims2use[1],to=lims2use[2],by=0.02),1)) + 
  guides(fill = guide_colourbar(nbin = 100)) 
# ggsave(filename = file.path(here(),"figures","diagnosis_maineffects_unthresholded_Hurst.pdf"), plot = p)


p2 = ggseg(.data = output_res, 
          mapping=aes(fill=TDm_cov), 
          position="stacked", 
          atlas=glasser) + 
  ggtitle("TD Male CoV") + 
  theme(plot.title = element_text(hjust = 0.5)) + 
...
p3 = ggseg(.data = output_res, 
          mapping=aes(fill=ASDf_cov), 
          position="stacked", 
          atlas=glasser) + 
  ggtitle("ASD Female CoV") + 
  theme(plot.title = element_text(hjust = 0.5)) + 
  guides(fill=guide_legend(title="SD")) + 
  scale_fill_gradient2(midpoint=0, low="blue", mid="white", high="red", limits=lims2use, breaks = round(seq(from=lims2use[1],to=lims2use[2],by=0.02),1)) + 
  guides(fill = guide_colourbar(nbin = 100)) 
...
p4 = ggseg(.data = output_res, 
          mapping=aes(fill=ASDm_cov), 
          position="stacked", 
          atlas=glasser) + 
  ggtitle("ASD Male CoV") + 
  theme(plot.title = element_text(hjust = 0.5)) + 
  guides(fill=guide_legend(title="SD")) + 
  scale_fill_gradient2(midpoint=0, low="blue", mid="white", high="red", limits=lims2use, breaks = round(seq(from=lims2use[1],to=lims2use[2],by=0.02),1)) + 
  guides(fill = guide_colourbar(nbin = 100)) 
